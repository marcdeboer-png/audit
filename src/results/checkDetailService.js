...
export function getCheckDetail(db, runId, checkResultId, options = {}) {
  ...
  return {
    ...
    rawPriority: checkResult.originalPriority || checkResult.priority,
    rawFindingType: checkResult.originalFindingType || checkResult.findingType,
    ...
    standardVersion: checkResult.standardVersion || null,
    standardStatus: checkResult.standardStatus || null,
    standardUsage: checkResult.standardUsage || null,
    standardSeverity: checkResult.standardSeverity ?? null,
    standardScoreEffect: checkResult.standardScoreEffect || null,
    standardFindingType: checkResult.standardFindingType || null,
    diagnosticOnly: Boolean(checkResult.diagnosticOnly),
    disabled: Boolean(checkResult.disabled),
    standardApplicability: checkResult.standardApplicability || null,
    standardNotApplicableRule: checkResult.standardNotApplicableRule || null,
    standardReviewStatus: checkResult.standardReviewStatus || null,
    standardRollupRole: checkResult.standardRollupRole || null,
    standardPatternRole: checkResult.standardPatternRole || null,
    standardScoreOwnerCheckId: checkResult.standardScoreOwnerCheckId || null,
    affectedCount: checkResult.affectedCount || 0,
    sampleUrls: checkResult.sampleUrls,
    evaluationState: checkResult.evaluationState,
    evidenceClass: checkResult.evidenceClass || null,
    executionStatus: checkResult.executionStatus || null,
    evidenceStatus: checkResult.evidenceStatus || null,
    evaluationStatus: checkResult.evaluationStatus || checkResult.evaluationState,
    coverageStatus: checkResult.coverageStatus || null,
    coverageUnitKey: checkResult.coverageUnitKey || null,
    coverageWeight: checkResult.coverageWeight ?? null,
    coverageReason: checkResult.coverageReason || null,
    availabilitySemanticsVersion: checkResult.availabilitySemanticsVersion || null,
...
function pageRenderProvenanceForRows(db, runId, rows) {
  ...
  return db.prepare(`
    SELECT p.url, p.renderStatus, p.settlingStatus, p.metadataProvenanceComplete,
      ...
      urm.renderDecisionReasonJson, urm.renderSignalsJson, urm.renderNegativeSignalsJson,
      urm.renderSignalContributionsJson, urm.renderRecommendationScore, urm.renderRecommendationThreshold,
      urm.renderCheckRequirementsJson, urm.budgetStatusJson,
