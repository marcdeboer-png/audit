...
function renderCheckDetail(detail) {
  ...
  const rootCauseMemberships = Array.isArray(detail.rootCauseMemberships) ? detail.rootCauseMemberships : [];
  const rootCauseSummary = rootCauseMemberships.length
    ? rootCauseMemberships.map((root) => `${root.rootCauseId || 'Root Cause'} · ${root.primaryCheckId || detail.checkId} · ${root.appliedPenalty ?? 0} Punkte · ${root.deduplicationReason || 'deterministische Zuordnung'}`).join(' | ')
    : 'Für dieses Finding ist kein persistierter Root-Cause-Cluster vorhanden.';
